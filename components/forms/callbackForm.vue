<template>
	...
	>
		...
		<el-form-item prop="inn" v-if="inputInn">
			<el-input
				type="text"
				v-model="mainForm.inn"
				placeholder="ИНН"
				maxlength="12"
				v-mask="'############'"
				show-word-limit
		...
		<el-form-item prop="model" v-if="model">
			<el-select v-model="mainForm.model" placeholder="Выберите модель">
				<el-option label="LX" value="LX">LX</el-option>
				<el-option label="TXL" value="TXL">TXL</el-option>
				<el-option label="VX" value="VX">VX</el-option>
			</el-select>
		</el-form-item>
		<el-form-item>
			<el-button
...
export default {
	directives: { mask },
	props: ["button", "input-name", "input-inn", "model"],
	data() {
		return {
			name: "mainForm",
			loading: false,
